...
func TestSetAddress(t *testing.T) {
	...
	err2 := store.SetAddress(id, newAddress)
	assert.NoError(t, err2)
...
func TestSetStatus(t *testing.T) {
	...
	// set status
	err2 := store.SetStatus(id, ParcelStatusSent)
	assert.NoError(t, err2)
	// check
	res, _ := store.Get(id)
...
func TestGetByClient(t *testing.T) {
	...
	for _, parcel := range storedParcels {
		...
		OldlParcel, exists := parcelMap[parcel.Number]
		assert.True(t, exists, parcel.Number)

		assert.Equal(t, OldlParcel.Client, parcel.Client)
		assert.Equal(t, OldlParcel.Status, parcel.Status)
		assert.Equal(t, OldlParcel.Address, parcel.Address)
		assert.Equal(t, OldlParcel.CreatedAt, parcel.CreatedAt)
	}
}
